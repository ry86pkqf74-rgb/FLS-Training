...
cp data/training/youtube_sft_v3/yt_train.jsonl $WS/yt_train.jsonl
cp data/training/youtube_sft_v3/yt_val.jsonl   $WS/yt_val.jsonl
cp data/training/youtube_sft_v3/yt_test.jsonl  $WS/yt_test.jsonl
wc -l $WS/yt_*.jsonl
...
tmux new-session -d -s $SESSION "cd $WS/FLS-Training && python3 scripts/047_train_qwen_vl_v5.py 2>&1 | tee $WS/trainv5.log"
echo "Launched v5 training in tmux session '$SESSION'"
