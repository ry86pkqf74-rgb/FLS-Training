...
cp data/training/youtube_sft_v6/yt_train.jsonl $WS/yt_train.jsonl
cp data/training/youtube_sft_v6/yt_val.jsonl   $WS/yt_val.jsonl
cp data/training/youtube_sft_v6/yt_test.jsonl  $WS/yt_test.jsonl
wc -l $WS/yt_*.jsonl
...
nvidia-smi | head -15
SESSION=trainv10
if tmux has-session -t $SESSION 2>/dev/null; then echo "session exists"; exit 0; fi
