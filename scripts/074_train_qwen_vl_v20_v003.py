#!/usr/bin/env python3
"""v20 multimodal LoRA — resumes from v19 with the auto-fail-relabeled, FP-tightened dataset.

Goal: keep v19's 100% schema compliance + 100% task accuracy + 100% critical-error
recall, while improving (a) absolute score MAE on Task 5 (because the labels now
agree with the v003 auto-fail rule) and (b) critical-error precision (because
the dataset only emits critical_errors entries when frame_evidence has ≥2
frames or the type is an absolute auto-fail).

Hyperparameters vs v19:
* Resume from v19 (adapter ships in from local Mac).
* 3 epochs (vs v19's 4) — the new dataset is the same size but the labels are
  cleaner, so the model needs less compute to find the right minimum.
* LR 1.5e-5 — even lower than v19's 2e-5 since we're already on a good adapter.
* Same LoRA dropout 0.1.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--data-dir", default="/workspace/v003_multimodal_v20")
    p.add_argument("--frames-dir", default="/workspace/v003_frames")
    p.add_argument("--output-dir", default="/workspace/v20_lora_output")
    p.add_argument("--base-adapter", default="/workspace/v19_lora_output/final_adapter")
...
def main() -> None:
    ...
    print(f"[{datetime.now()}] v20 (v003) multimodal LoRA training")
    ...
    print(f"\n[{datetime.now()}] Starting v20 training...")
